...
// Helper to validate that only valid IDs are returned
export function getPackagesByIds(ids: number[]): TravelPackage[] {
  return ids
    .map(id => INVENTORY.find(pkg => pkg.id === id))
    .filter((pkg): pkg is TravelPackage => pkg !== undefined);
}
